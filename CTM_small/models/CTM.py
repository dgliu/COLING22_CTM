...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        lstm_size = trial.suggest_categorical('lstm_size', [128])
...
class CTM(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('CTM'):
            ...
            with tf.variable_scope('Loss'):
                ...
                loss_1 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task1_head), labels=self.get_masked_tensor(gold_matrix_law))
                loss_2 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task2_head), labels=self.get_masked_tensor(gold_matrix_accu))
                loss_3 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task3_head), labels=self.get_masked_tensor(gold_matrix_time))
                ...
                loss_1 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task1_tail, 'tail'), labels=self.get_masked_tensor(gold_matrix_law, 'tail'))
                loss_2 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task2_tail, 'tail'), labels=self.get_masked_tensor(gold_matrix_accu, 'tail'))
                loss_3 = tf.nn.softmax_cross_entropy_with_logits(logits=self.get_masked_tensor(output_task3_tail, 'tail'), labels=self.get_masked_tensor(gold_matrix_time, 'tail'))
                ...
                self.category_loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=gold_matrix_category, logits=output_task6))
                ...
                self.triplet_loss1 = tf.reduce_sum(tf.maximum(0.0, 0.5 + d_pos - d_neg))
                ...
                self.triplet_loss2 = tf.reduce_sum(tf.maximum(0.0, 0.3 + d_pos - d_neg))
                ...
                tf.add_to_collection('losses_4', tf.contrib.layers.l2_regularizer(0.0001)(self.time_loss_tail))
                tf.add_to_collection('losses_5', tf.contrib.layers.l2_regularizer(0.0001)(self.time_loss_tail))
    ...
    def get_comparable_cases(self, inputs, law_labels_input, accu_labels_input, fact, law_labels, accu_labels, mode=0,
                             seed=0):
        setup_seed(seed)
        comparable_cases = []
        ...
        for i in range(len(inputs)):
            if mode == 0:
                ...
                index = np.where(_law_labels == law_labels_input[i])[0]
                _index = np.setdiff1d(index, i)
                sample_index = np.random.choice(_index, 1)[0]
                comparable_cases.append(fact[sample_index])
            elif mode == 1:
                # mode 1: similar_accu_cases
                index = np.where(_accu_labels == accu_labels_input[i])[0]
                _index = np.setdiff1d(index, i)
                sample_index = np.random.choice(_index, 1)[0]
                comparable_cases.append(fact[sample_index])
            elif mode == 2:
                # mode 2: dissimilar_law_cases and dissimilar_accu_cases
                index = np.where((_accu_labels != accu_labels_input[i]) & (
                        np.isin(_law_labels, self._tail_law_index, invert=True)) & (
                        np.isin(_accu_labels, self._tail_accu_index, invert=True)))[0]
            ...
            for epoch in range(max_epoch):
                for i in range(train_step):
                    if i == train_step - 1:
                        ...
                        similar_law_cases = self.get_comparable_cases(inputs, law_labels_input, accu_labels_input,
                                                                      fact_train, law_labels_train, accu_label_train, 0,
                                                                      seed)
                        similar_accu_cases = self.get_comparable_cases(inputs, law_labels_input, accu_labels_input,
                                                                       fact_train, law_labels_train, accu_label_train,
                                                                       1,
                                                                       seed)
                        dissimilar_cases = self.get_comparable_cases(inputs, law_labels_input, accu_labels_input,
                                                                     fact_train, law_labels_train, accu_label_train,
                                                                     2, seed)
                        similar_law_input = np.array(similar_law_cases, dtype='int32')
                        similar_accu_input = np.array(similar_accu_cases, dtype='int32')
                        dissimilar_input = np.array(dissimilar_cases, dtype='int32')
                    else:
